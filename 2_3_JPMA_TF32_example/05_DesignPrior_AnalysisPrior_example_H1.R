#----------------------------------------------------------------#
#Title : Design prior and Analysis prior (simple example under H1)
#Date  : 2024/11/5
#----------------------------------------------------------------#
...
SOC <- 0.2          #ORR of SOC setting (H0)
TPP <- 0.5          #ORR of TPP setting (H1)
...
prob_infbeta <- extraDistr::dbbinom(x, N, alpha = 50, beta = 50, log = FALSE)
...
prob_vagbeta <- extraDistr::dbbinom(x, N, alpha = 5, beta = 5, log = FALSE)
...
# Save results
ggsave("./output/08_plot_dirac_H1.png", plot = plot_dirac)
ggsave("./output/09_plot_infbeta_H1.png", plot = plot_infbeta)
ggsave("./output/10_plot_vagbeta_H1.png", plot = plot_vagbeta)

capture.output(result, file = "./output/11_probability_H1.txt")
